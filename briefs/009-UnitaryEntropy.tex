...
\title{Schroedinger equation is equivalent to information conservation}
\author{Gabriele Carcassi}
\category{Quantum mechanics}
\tags{Schroedinger equation, entropy, unitary evolution}
\begin{document}
	\maketitle
...
\begin{condition}[Information preservation]{DR-INFO}
	The evolution is differentiable and preserves von Neumann entropy: $S(\mathcal{E}(\rho)) = S(\rho)$ for all $\rho \in E(\mathcal{H})$.
\end{condition}

Note that \ref{DR-SCEQ} respects the following condition:
\begin{condition}[Affine evolution]{EV-AFF}
	The time evolution map $\mathcal{E} : E(\mathcal{H}) \to E(\mathcal{H})$ is affine: $\mathcal{E}(\sum_i p_i \rho_i) = \sum_i p_i \mathcal{E}(\rho_i)$.
\end{condition}
...
\begin{thrm}[Schrödinger equation is information preservation]
	Over \ref{QST} and \ref{EV-AFF}, conditions \ref{DR-SCEQ} and \ref{DR-INFO} are equivalent.
\end{thrm}
\begin{proof}
